...
NOTE: Since this is a dynamic router, channels are created as needed; therefore, by default the `defaultOutputChannel`
...
`values` and `destinations` are used to map the evaluation results to an actual destination name; the must have the same
number of elements.
For example: `values = foo,bar` and `destinations = baz,qux` - if the expression or script returns `foo`, the message
